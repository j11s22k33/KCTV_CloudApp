...
W.defineModule(["mod/Util", "comp/PopupButton"], function(util, buttonComp) {
	...
	function create(){
		...
		if((detail.pr.seriesId || detail.pr.assetId) && W.StbConfig && W.StbConfig.cugType != "accommodation") {
			_comp.btns[0] = buttonComp.create(644-26.5, 579-324, W.Texts["close"], 133, null, null, 18);
			_comp.btns[1] = buttonComp.create(502-26.5, 579-324, W.Texts["relation_vod"], 133, null, null, 18);
			_comp.add(_comp.btns[0].getComp());
			_comp.add(_comp.btns[1].getComp());
		} else {
			_comp.btns[0] = buttonComp.create(574-26.5, 579-324, W.Texts["close"], 133, null, null, 18);
			_comp.add(_comp.btns[0].getComp());
		}
    ...
    return W.Popup.extend({
    	onStart: function(_param) {
			_this = this;
			W.log.info(_param)
    		W.log.info("PastMoreInfoPopup onStart");
    		detail = _param.data;
