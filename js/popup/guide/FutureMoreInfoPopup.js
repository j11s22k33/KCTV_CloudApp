...
W.defineModule(["mod/Util", "comp/PopupButton"], function(util, buttonComp) {
	...
	function create(){
		...
		if(util.findReserveProgram(detail.pr.sourceId, detail.pr.start_time, detail.pr.title)) {
			_comp._title.add(new W.Image({display:"inline-block", position:"relative", src:"img/info_resv.png", width:61, height:21, y:-5,
				marginRight:"5px", marginLeft:"4px"}));
			maxWidth -= 70;
		}
		_comp._title._span.setStyle({"max-width":maxWidth+"px"});
		//_comp.add(new W.Span({x:381, y:69, width:"100px", height:"18px", textColor:"rgb(181,181,181)", "font-size":"16px",
		//	className:"font_rixhead_light", text:"정보 더보기", textAlign:"center"}));
		_comp.add(_comp._title);
		
		_comp.btns = [];
		if((detail.pr.seriesId || detail.pr.assetId) && W.StbConfig && W.StbConfig.cugType != "accommodation") {
			_comp.btns[0] = buttonComp.create(574-26.5, 579-324,
				util.findReserveProgram(detail.pr.sourceId, detail.pr.start_time, detail.pr.title) ? W.Texts["cancel_rev"] : W.Texts["reservation_prog"], 133, null, null, 18);
			_comp.btns[1] = buttonComp.create(715-26.5, 579-324, W.Texts["close"], 133, null, null, 18);
			...
			_comp.btns[0] = buttonComp.create(502-26.5, 579-324,
				util.findReserveProgram(detail.pr.sourceId, detail.pr.start_time, detail.pr.title) ? W.Texts["cancel_rev"] : W.Texts["reservation_prog"], 133, null, null, 18);
			_comp.btns[1] = buttonComp.create(644-26.5, 579-324, W.Texts["close"], 133, null, null, 18);
			_comp.add(_comp.btns[0].getComp());
			_comp.add(_comp.btns[1].getComp());
		}
		...
		_comp._startTime = new W.Div({x:583-26.5, y:438-324, textAlign:"left", width:"100px", "font-size":"17px", textColor:"rgba(181,181,181,0.5)", fontFamily:"RixHeadM",
		...
		_comp._endTime = new W.Div({x:636-26.5, y:438-324, textAlign:"left", width:"100px", "font-size":"17px", textColor:"rgba(181,181,181,0.5)", fontFamily:"RixHeadM",
    ...
    return W.Popup.extend({
    	onStart: function(_param) {
			_this = this;
			W.log.info(_param)
    		W.log.info("FutureMoreInfoPopup onStart");
    		detail = _param.data;
    	...
    	onStop: function() {
    		W.log.info("FutureMoreInfoPopup onStop");
    		state = STATE_BTNS;
    		cIdx = 0;
    	},
    	onKeyPressed : function(event) {
    		W.log.info("FutureMoreInfoPopup onKeyPressed "+event.keyCode);
    		...
    		case W.KEY.ENTER:
				if(state == STATE_BTNS){
					if(index == 0) {
						W.PopupManager.closePopup(this, {action:W.PopupManager.ACTION_OK, type:"reserve", data:detail});
					} else if(index == 1) {
						W.PopupManager.closePopup(this, {action:W.PopupManager.ACTION_CLOSE});
					} else if(index == 2) {
