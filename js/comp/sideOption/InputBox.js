 ...
 */
W.defineModule(["mod/Util", "comp/sideOption/Button"], function(util, Button) {
    function InputBox() {
        var _this = this;
        ...
        var create = function(data){
            ...
            _this.data = data;
            _comp = new W.Div({position:"relative", marginTop:"8px", x:data.x, y:data.y, width:244, height:155});
            ...
            _comp._placeHolder = new W.Div({x:10, y:0, width:224, height:56, text:W.Texts["series_input_guide"], textColor:"#837A77", "font-size":"14px", "letter-spacing":"-0.7px", lineHeight:"56px", fontFamily:"RixHeadL", textAlign:"center"});
            _comp.add(_comp._placeHolder);
            _comp._text = new W.Div({x:10, y:0, width:224, height:56, textColor:"#FFFFFF", "font-size":"18px", "letter-spacing":"-0.9px", lineHeight:"56px", fontFamily:"RixHeadB", textAlign:"center"});
            _comp.add(_comp._text);

            var lastEpiNo = 0;
            for(var i=0; i < _this.data.episodeList.length; i++){
            	var epiNo = Number(_this.data.episodeList[i].episodeNum);
            	if(lastEpiNo < epiNo){
            		lastEpiNo = epiNo;
            	}
            }
            ...
            _comp._countText = new W.Div({x:-20, width:284, y:68, "font-size":"16px", textColor:"#B5B5B5", "letter-spacing":"-0.8px", opacity:0.75, textAlign:"center"});
            
            for(var i=0; i < tmpArr.length; i++){
            	if(tmpArr[i] == "total_count"){
            		_comp._countText._count = new W.Span({text:lastEpiNo, textColor:"#EDA802", position:"relative", fontFamily:"RixHeadL", "font-size":"16px", textAlign:"left"});
                    _comp._countText.add(_comp._countText._count);
            	}else{
            		_comp._countText.add(new W.Span({text:tmpArr[i], position:"relative", fontFamily:"RixHeadL", "font-size":"16px", textAlign:"left"}));
            	}
            }
            _comp.add(_comp._countText);

            _comp._errorText = new W.Div({x:-20, width:284, y:68, "font-size":"16px", textColor:"#E3542E", "letter-spacing":"-0.8px", opacity:0.9, textAlign:"center", display:"none"});
            _comp._errorText.add(new W.Span({text:W.Texts["series_input_guide2"], position:"relative", fontFamily:"RixHeadM", "font-size":"16px", textAlign:"left"}));
        ...
        var setText = function() {
            ...
            if(_this.value.length > 0) {
            	...
            	if(isExist){
            		...
            		var isExist2 = false;
            		for(var i=0; i < _this.data.episodeList.length; i++){
                		if(_this.data.episodeList[i].episodeNum.indexOf(_this.value) == 0){
                			isExist2 = true;
                			break;
                		}
                	}
            		if(!isExist2){_this.value = "";
	                    setText();
