...
W.defineModule(["mod/Util", "comp/setting/Templete", "comp/setting/Button", "comp/setting/Box"], function(util, Templete, Button, Box) {
    function VODContinuousPlay() {
        ...
        var create = function(title){
            ...
            _comp._templete = _this.templete.getComp({title:title, desc:W.Texts["vod_continuous_guide"]});
            ...
            _this.box[0] = new Box();
            _comp._box[0] = _this.box[0].getComp({x:159, y:325, width:244, text:W.Texts["time_restricted_option1"]});
            _comp.add(_comp._box[0]);
            _this.box[1] = new Box();
            _comp._box[1] = _this.box[1].getComp({x:159, y:395, width:244, text:W.Texts["play_repeat"]});
            _comp.add(_comp._box[1]);
            _this.box[2] = new Box();
            _comp._box[2] = _this.box[2].getComp({x:159, y:465, width:244, text:W.Texts["no_setting"]});
        ...
        this.setData = function(_data) {
            ...
            if(_data && _data.data) {
                selectedIndex = index = _data.data ? _data.data-1 : 0;
            }
        ...
        this.operate = function(event) {
            ...
            switch (event.keyCode) {
                ...
                case W.KEY.DOWN:
                    if(mode == MODE_TYPE.BOX) {
                        _this.box[index].unFocus();
                        index = (index >= _this.box.length-1 ? index : index+1);
                        _this.box[index].setFocus();
                    } else {
                        _this.btn[btnIndex].unFocus();
                        btnIndex = ++btnIndex%2;
                ...
                case W.KEY.ENTER:
                    if(mode == MODE_TYPE.BOX) {
                        ...
                        if(btnIndex == 0) {
                            saveCallback(true, selectedIndex ? selectedIndex+1 : 1);
                        } else {
                            saveCallback(false);
                        }
                    }
        ...
        this.componentName = "VODContinuousPlay";
    }
    return VODContinuousPlay;
});