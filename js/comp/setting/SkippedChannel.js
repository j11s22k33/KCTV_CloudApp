...
W.defineModule(["mod/Util", "comp/setting/Templete", "comp/setting/Button", "comp/setting/SelectBox", "comp/Scroll"], function(util, Templete, Button, SelectBox, Scroll) {
    function SkippedChannel() {
        ...
        var pageIndex = 0;
        var _comp;
        var notSubscribedChannels;
        var maxSelectCount = 150;
        ...
        var skipUnsubscribed = false;

        var MODE_TYPE = Object.freeze({BOX:0, BTN:1, SCROLL:2});

        var create = function(title){

            getNotSubscribedChannels({offset:0, limit:0});

            mode = MODE_TYPE.BOX;
            _comp = new W.Div({});
            _this.templete = new Templete();
            _comp._templete = _this.templete.getComp({title:title, desc:W.Texts["skip_ch_guide"]});
            ...
            for(var i = 0; i < W.CH_LIST.length; i++) {
                if(W.CH_LIST[i] && W.CH_LIST[i].channelType && W.CH_LIST[i].channelType.code){
                    if(W.CH_LIST[i].channelType.code == "2" || W.CH_LIST[i].channelType.code == "60" || W.CH_LIST[i].channelType.code == "61") {
            ...
            _comp.add(new W.Div({text:W.Texts["now"], x:1032, y:321, width:100, textColor:"#C7C7C7", fontFamily:"RixHeadL", "font-size":"18px", textAlign:"left", "letter-spacing":"0.18px"}));
            ...
            _comp.add(new W.Div({text:W.Texts["blocked_ch_select"].replace("50", maxSelectCount), x:1004, y:354, width:150, textColor:"#B5B5B5", fontFamily:"RixHeadL", "font-size":"14px", textAlign:"left", "letter-spacing":"-0.7px"}));
            ...
            _comp._btn[1] = _this.btn[1].getComp({x:988, y:435, text:W.Texts["not_subcribed_ch"] + " " + W.Texts["select"], fontSize:16});
            _comp.add(_comp._btn[1]);
            _this.btn[2] = new Button();
            _comp._btn[2] = _this.btn[2].getComp({x:988, y:483, text:W.Texts["reset"]});
            _comp.add(_comp._btn[2]);
            _this.btn[3] = new Button();
            _comp._btn[3] = _this.btn[3].getComp({x:988, y:531, text:W.Texts["cancel"]});
        ...
        var getNotSubscribedChannels = function(param) {
            W.Loading.start();
            sdpDataManager.getNotSubscribedChannels(cbGetNotSubscribedChannels, param);
        };

        var cbGetNotSubscribedChannels = function(isSuccess, result) {
            if(isSuccess) {
                notSubscribedChannels = result.data;
                W.Loading.stop();
            } else {
        ...
        this.setData = function(_data) {
            if(_data && _data.data) {
                ...
                favData = _data.data.favorite;

                var idx;
                for(var i = 0; i < _data.data.skipped.sourceIds.length; i++) {
                    idx = data.findIndex(function(object){return object.sourceId == _data.data.skipped.sourceIds[i]});
                    if(idx > -1) {
                ...
                if(_data.data.skipUnsubscribed) {
                    skipUnsubscribed = true;
                    _this.btn[1].setText(W.Texts["not_subcribed_ch"] + " " + W.Texts["unselect"]);
                } else {
                    skipUnsubscribed = false;
                    _this.btn[1].setText(W.Texts["not_subcribed_ch"] + " " + W.Texts["select"]);
                }
            }
            ...
            switch (event.keyCode) {
                ...
                case W.KEY.UP:
                    if(mode == MODE_TYPE.BOX) {
                        ...
                        _this.btn[btnIndex].unFocus();
                        btnIndex = btnIndex-1 < 0 ? 3 : btnIndex-1;
                        _this.btn[btnIndex].setFocus();
                    }
                    break;
                case W.KEY.DOWN:
                    if(mode == MODE_TYPE.BOX) {
                        ...
                        _this.btn[btnIndex].unFocus();
                        btnIndex = btnIndex+1 > 3 ? 0 : btnIndex+1;
                        _this.btn[btnIndex].setFocus();
                    }
                    break;
                case W.KEY.ENTER:
                    if(mode == MODE_TYPE.BOX) {
                        if(selectedArray.includes(data[index])) {
                            ...
                            if(selectedArray.length >= maxSelectCount) {
                                W.PopupManager.openPopup({
                                    ...
                                    boldText:W.Texts["setting_skip_limit"].replace("@max_count@", maxSelectCount),
                                    button:[W.Texts["close"]]}
                                );
                            } else if(favData.sourceIds.indexOf(parseInt(data[index].sourceId)) > -1) {
                                W.PopupManager.openPopup({
                                    childComp:_this,
                                    popupName:"popup/FeedbackPopup",
                                    title:W.Texts["setting_skip_restrict"]}
                                );
                            } else {
                        ...
                        if(btnIndex == 0) {
                            ...
                            if(skipUnsubscribed) {
                                skipUnsubscribed = false;
                                var idx;
                                for(var i = 0; i< notSubscribedChannels.length; i++) {
                                    idx = data.findIndex(function(object){return object.sourceId == notSubscribedChannels[i].sourceId});
                                    if(idx > -1) {
                                        _this.box[idx].unSelected();
                                        if(selectedArray.indexOf(data[idx]) > -1) selectedArray.splice(selectedArray.indexOf(data[idx]),1);
                                    }
                                }
                                _comp._selectedNumber.setText(selectedArray.length);
                                _this.btn[1].setText(W.Texts["not_subcribed_ch"] + " " + W.Texts["select"]);
                                ...
                                for(var i = 0; i< notSubscribedChannels.length; i++) {
                                    ...
                                    if(idx > -1) {
                                        _this.box[idx].setSelected();
                                        if(selectedArray.indexOf(data[idx]) < 0) selectedArray.push(data[idx]);
                                    }
                                }
                                _comp._selectedNumber.setText(selectedArray.length);
                                _this.btn[1].setText(W.Texts["not_subcribed_ch"] + " " + W.Texts["unselect"]);
            ...
            if (desc) {
                if (desc.popupName == "popup/setting/SelectChannelConfirmPopup") {
                    if (desc.action == W.PopupManager.ACTION_OK) {
                        ...
                        saveCallback(true, {sourceIds : requestArray, skipUnsubscribed : skipUnsubscribed});
                    } else if(desc.idx == 2){
                        saveCallback(false);
                    }
                }
        ...
        this.componentName = "SkippedChannel";
    }
    return SkippedChannel;
});