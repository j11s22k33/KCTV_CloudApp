...
W.defineModule(["mod/Util", "comp/setting/Templete", "comp/setting/Button", "comp/setting/SelectBox", "comp/Scroll"], function(util, Templete, Button, SelectBox, Scroll) {
    function FavoriteChannel() {
        ...
        var create = function(title){
            ...
            _comp._templete = _this.templete.getComp({title:title, desc:W.Texts["fav_ch_guide"]});
        ...
        this.setData = function(_data) {
            if(_data && _data.data) {
                W.log.info(_data.data)
                selectedArray = [];

                skipData = _data.data.skipped;

                var idx;
                for(var i = 0; i < _data.data.favorite.sourceIds.length; i++) {
                    idx = data.findIndex(function(object){return object.sourceId == _data.data.favorite.sourceIds[i]});
                    if(idx > -1) {
                        ...
                        if(selectedArray.includes(data[index])) {
                            ...
                            if(selectedArray.length >= maxSelectCount) {
                                W.PopupManager.openPopup({
                                    ...
                                    boldText:W.Texts["setting_fav_limit"].replace("@max_count@", maxSelectCount),
                                    button:[W.Texts["close"]]}
                                );
                            } else if(skipData.sourceIds.indexOf(parseInt(data[index].sourceId)) > -1) {
                                W.PopupManager.openPopup({
                                    childComp:_this,
            ...
            if (desc) {
                if (desc.popupName == "popup/setting/SelectChannelConfirmPopup") {
                    if (desc.action == W.PopupManager.ACTION_OK) {
                        ...
                        saveCallback(true, {sourceIds : requestArray});
                    } else if(desc.idx == 2){
                        saveCallback(false);
                    }
                }
        ...
        this.componentName = "FavoriteChannel";
    }
