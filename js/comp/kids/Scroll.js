W.defineModule([ "mod/Util"], function(util) {
    function Scroll(type, data) {
        ...
        var create = function(_totalPage, _initialPageIdx, _callback){
            ...
            _comp = new W.Div({x:0, y:-46, width:36, height:178});

            _comp._firstPage = new W.Div({text:"1", x:0, y:0, width:35, height:35, lineHeight:"35px", textColor:"#FFFFFF", opacity:0.5,
                fontFamily:"RixHeadB", "font-size":"18px", textAlign:"center", "letter-spacing":"-0.9px"});
            _comp.add(_comp._firstPage);
            _comp._lastPage = new W.Div({text:totalPage, x:0, y:236, width:35, height:35, lineHeight:"35px", textColor:"#FFFFFF", opacity:0.5,
                fontFamily:"RixHeadB", "font-size":"18px", textAlign:"center", "letter-spacing":"-0.9px"});
            _comp.add(_comp._lastPage);

            _comp._line = new W.Div({x:17, y:0+35, width:1, height:201, color:"#FFFFFF", opacity:0.3});
            _comp.add(_comp._line);
            _comp._line2 = new W.Div({x:17, y:201+35, width:1, height:0, color:"#FFFFFF", opacity:0.3});
            _comp.add(_comp._line2);

            _comp._focusArea = new W.Div({x:0, y:0, width:35, height:35});
            _comp._focusArea._normal = new W.Div({x:0, y:0, width:35, height:35, lineHeight:"35px", text:curPageIdx+1, textColor:"#FFFFFF", fontFamily:"RixHeadB",
                "font-size":"18px", textAlign:"center", "letter-spacing":"-0.9px", backgroundImage:"url('img/scr_d_kids.png')"});
            _comp._focusArea.add(_comp._focusArea._normal);
            _comp._focusArea._focus = new W.Div({x:0, y:0, width:35, height:35, lineHeight:"35px", text:curPageIdx+1, textColor:"#FFFFFF", fontFamily:"RixHeadB",
                "font-size":"18px", textAlign:"center", "letter-spacing":"-0.9px", backgroundImage:"url('img/scr_f.png')", display:"none"});
        ...
        var setPage = function(idx, needCallback) {
            ...
            if(idx == 0) {
                ...
                _comp._line.setStyle({y:0+35, height:201});
                _comp._line2.setStyle({y:201+35, height:0});
                ...
                _comp._line.setStyle({y:0+35, height:_y-35});
                _comp._line2.setStyle({y:_y+35, height:201-_y});
            }
