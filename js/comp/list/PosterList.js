W.defineModule("comp/list/PosterList", ["mod/Util", "comp/list/Poster", "comp/list/Banner"], function(util, Poster, Banner) {
    function PosterList(_param) {
        ...
        var create = function(){
            ...
            if(type == PosterList.TYPE.SEARCH) {
                ...
                posterGapX = 123;
                posterGapY = 249;
            } else if(type == PosterList.TYPE.PURCHASE) {
                posterType = Poster.TYPE.W136;
                posterGapX = 151;
                posterGapY = 284;
            } else if(type == PosterList.TYPE.WATCHEDLIST) {
                posterType = Poster.TYPE.W136;
                posterGapX = 151;
                posterGapY = 284;
            } else {
            ...
            _this.firstRowCount = 7;
            if(type == PosterList.TYPE.SEARCH || type == PosterList.TYPE.PURCHASE || type == PosterList.TYPE.WATCHEDLIST || !bannerData || (bannerData && bannerData.length < 1)) {
                _this.bannerType = Banner.TYPE.NONE;
                _this.firstRowCount = 7;
            } else if(bannerData && bannerData[0].bannerSize == 3) {
                _this.bannerType = Banner.TYPE.TYPE_1;
                _this.firstRowCount = 1;
                bannerData[0].type = "banner";
                data.splice(0,0,bannerData[0]);
            } else if(bannerData && bannerData[0].bannerSize == 2) {
            ...
            } else if(bannerData && bannerData[0].bannerSize == 1 && bannerData.length == 1) {
            ...
            for(var i = 0; i < total/*data.length*/; i++) {
                ...
                if((_this.bannerType != Banner.TYPE.NONE) && (i < _this.firstRowCount)) {
                    if(data[i].type == "banner") {
                        ...
                        else if(_this.bannerType == Banner.TYPE.TYPE_2) _x += 529;
                        ...
                        if(data[i]) {
                            _this.posters[i] = new Poster({type:posterType, data:data[i], textAlign:(_this.nextRowIdx+i)%COLUMN_COUNT>=COLUMN_COUNT-2 ? "right":"",
                                ...
                                isClearPin : _this.isClearPin, isLiveTriggerDown:_param.isLiveTriggerDown});
           ...
           /* setTimeout(function(){
                for(var i = 0; i < total/!*data.length*!/; i++) {
                    _comp._postersComp[i]._normalDiv._poster.setStyle({visibility:""});
                    _comp._postersComp[i]._focusDiv._poster.setStyle({visibility:""});
                }
            },500);*/
        };
        ...
        var operateList = function(event) {
            switch (event.keyCode) {
                case W.KEY.RIGHT:
                    ...
                    if(type == PosterList.TYPE.PLAYLIST) {
                ...
                case W.KEY.DOWN:
                    ...
                    if(_this.curRowIndex == _comp._postersRowComp.length-1) {
                        ...
                        if(_this.bannerType == Banner.TYPE.NONE) {
                            ...
                            else if((_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT < 5) _this.posterIndex = (_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT -1;
                            else _this.posterIndex = 4;
                        } else if(_this.bannerType == Banner.TYPE.TYPE_4) {
                            if((_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT < 6) _this.posterIndex = (_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT;
                            else _this.posterIndex = 5;
                        } else if(_this.bannerType == Banner.TYPE.TYPE_3) {
                            if((_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT < 2) _this.posterIndex = 0;
                            else _this.posterIndex = (_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT;
                        } else if(_this.bannerType == Banner.TYPE.TYPE_2) {
                            if((_this.posterIndex-_this.firstRowCount)%COLUMN_COUNT < 4) _this.posterIndex = 0;
                            else _this.posterIndex = 1;
                        } else if(_this.bannerType == Banner.TYPE.TYPE_1) {
                            _this.posterIndex = 0;
                        }
                        ...
                        if(_this.bannerType != Banner.TYPE.NONE && _this.posterIndex < _this.firstRowCount) {
                            if(_this.bannerType == Banner.TYPE.TYPE_5) {
                                ...
                                else if(_this.posterIndex < 4) _this.posterIndex = _this.posterIndex + 6;
                                else _this.posterIndex = 10;
                            } else  if(_this.bannerType == Banner.TYPE.TYPE_4) {
                                if(_this.posterIndex < 5) _this.posterIndex = _this.posterIndex + 6;
                                else _this.posterIndex = 11;
                            } else  if(_this.bannerType == Banner.TYPE.TYPE_3) {
                                if(_this.posterIndex < 1) _this.posterIndex = 6;
                                else _this.posterIndex = _this.posterIndex + 7;
                            } else  if(_this.bannerType == Banner.TYPE.TYPE_2) {
                                if(_this.posterIndex < 1) _this.posterIndex = 2;
                                else _this.posterIndex = 6;
                            } else  if(_this.bannerType == Banner.TYPE.TYPE_1) {
                                _this.posterIndex = 1;
                            }
        ...
        this.setSelectAll = function(limit) {
            setSelectAll(limit);
            return selectedArray;
        }
        ...
        this.componentName = "PosterList";
    }
    PosterList.TYPE = Object.freeze({MOVIE:0, SEARCH:1, PLAYLIST:2, PURCHASE:3, WATCHEDLIST:4, RECOMMEND:5, RANKING:6});
    return PosterList;
});