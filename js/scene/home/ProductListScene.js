 ...
 */
W.defineModule([ "mod/Util", "comp/home/ProductList", "comp/RightTimer"],
    function(util, productList, rightTimer) {
        var STATE_TOP = 0;
        var STATE_BANNER = 1;
        var STATE_MENU = 2;
        ...
        var _thisScene = "ProductListScene";
        var _this;
        var _parentDiv;
        var currMenu;
        var currComponent;
        ...
        return W.Scene.extend({
            onCreate : function(param) {
                ...
                _parentDiv._title = new W.Div({x:56,y:51, text:param.category.title, textColor:"rgba(255,255,255,1)",
                    fontFamily:"RixHeadM", "font-size":"27px", textAlign:"left", "letter-spacing":"-1.35px"})
                _parentDiv.add(_parentDiv._title);
                currComponent = productList.getNewComp();
                _currComponent = currComponent.create(_parentDiv, _this, param.category.baseId);
                _parentDiv._comp_area.add(_currComponent);
                
                _timer = rightTimer.getComp();
                
                _parentDiv._timer = new W.Div({x:1027, y:55, width:"200px", height:"117px"});
                _parentDiv._bg.add(_parentDiv._timer);
    			_timer.start(_parentDiv._timer);
