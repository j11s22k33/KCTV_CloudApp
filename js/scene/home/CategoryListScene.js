    ...
    function(util, categoryList, couponInfoComp) {
        ...
        return W.Scene.extend({
        	initCategory : function(category){
                W.log.info(_thisScene + " init");
            	this._parentDiv._title = new W.Div({x:56,y:51, text:category.title, textColor:"rgba(255,255,255,1)",
                    fontFamily:"RixHeadM", "font-size":"27px", textAlign:"left", "letter-spacing":"-1.35px"})
                this._parentDiv.add(this._parentDiv._title);

                this.currComponent = categoryList.getNewComp();
                this._currComponent = this.currComponent.create(undefined, category, this.isClearPin);
            ...
            onCreate : function(param) {
                ...
                if(param.category){
                	...
                	this.initCategory(param.category);
                }else{
                	var sdpDataManager = W.getModule("manager/SdpDataManager");
                	var reqData;
                	if(param.categoryId){
                		reqData = {categoryId:param.categoryId};
                	}else if(param.bctgrId){
                		reqData = {bctgrId:param.bctgrId};
                	}
                	sdpDataManager.getMenuDetail(function(result, data, _this){
                		if(result){
                			_this.category = data.data[0];
                			_this.initCategory(data.data[0]);
                		}
                	}, reqData, this);
                }
            ...
            onResume: function() {
            	this.currComponent.resume();
            },
            onRefresh: function() {
            	var currentIdx = this.currComponent.getCurrIndex();
            	W.log.info("currentIdx == " + currentIdx);
            	this.currComponent = categoryList.getNewComp();
                if(this._currComponent){
                	this._parentDiv._comp_area.remove(this._currComponent);
                }
                this._currComponent = this.currComponent.create(undefined, this.category, this.isClearPin, currentIdx);
                this._parentDiv._comp_area.add(this._currComponent);
