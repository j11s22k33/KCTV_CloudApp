 ...
 *
 */
W.defineModule([ "mod/Util", "comp/CouponInfo", "comp/vod/Detail", "comp/vod/RecommendList", "comp/vod/DetailModule"],
    function(util, couponInfoComp, detailComp, recommendComp, DetailModule) {
        var _thisScene = "VodDetailScene";
        var _this;
        ...
        function cbDetail(result, data){
			...
			if(_this.contentType == "Sasset"){
				...
				if(_this.detail.isSeries){
					...
					_this.reqData.seriesId = _this.detail.seriesId;
...
//							}
        ...
        function setBackground(){
        	if(_this.detail.iframeUrl){
        		_this._parentDiv._bg.add(new W.Image({x:0, y:0, width:"1280px", height:"720px", src:W.Config.IMAGE_URL + "/" + _this.detail.iframeUrl}));
        	}
        };
        ...
        return W.Scene.extend({
            onCreate : function(param) {
                ...
                this.detailModule = DetailModule.getNewComp(_this);
        		...
        		this.Recommend = recommendComp.getComp(this._parentDiv._recommend_area, this.contentType, this.param.isClearPin);
            		 ...
            		 if(desc.command == "goPurchase"){
                		 ...
                		 if(desc.result){
                 			...
                 			W.PopupManager.openPopup({
                                childComp:_this,
                                popupName:"popup/ErrorPopup",
                                code:desc.resultData ? desc.resultData.error.code : undefined,
                    			from : "SDP"}
                            );
                 		}
                	 }else if(desc.command == "backFromVodPlay"){
                		 _this.isBackFromVodPlay = true;
                	 }else if(desc.command == "fromVodPlay"){
                		 if(this.contentType == "series"){
                    		 _this.DetailComp.changeRecentIcon();
                		 }
                	 }else if(desc.command == "cancelPurchase"){
                		 var asset = desc.asset;
                		 
                	 }
            	 }else{
            		 if(!this.isDetail){
            			 W.entryPath.pop("recDetail");
            		 }
            	 }
            },
            		...
            		if (desc.popupName == "popup/purchase/PurchaseCompletePopup") {
        				...
        				if (desc.action == W.PopupManager.ACTION_OK) {
        					if(desc.idx < 2){
                				...
                				var param = {data:desc.desc, isClearPin:this.param.isClearPin};
