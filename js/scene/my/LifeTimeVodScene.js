/**
 * scene/LifeTimeVodScene
 *
 * [Flipbook] 3.1.1
 *
 * Author : jean511@alticast.com
 */
W.defineModule([ "mod/Util", "comp/my/LifeTimeVod", "comp/CouponInfo"],
    function(util, lifeTimeVod, couponInfoComp) {
        var STATE_TOP = 0;
        var STATE_BANNER = 1;
        var STATE_MENU = 2;
        ...
        var _thisScene = "LifeTimeVodScene";
        var _this;
        var _parentDiv;
        var currMenu;
        var currComponent;
        ...
        return W.Scene.extend({
            onCreate : function(param) {
                ...
                currComponent = lifeTimeVod.getNewComp();
                _currComponent = currComponent.create(null, {mode : 2});
            ...
            onPopupClosed: function(event){
                if(event.id == "popup_MiniDetailPopup"){

                }else if(event.id == "popup_ProfileWatchListPopup"){
                    if(event.param.action == W.PopupManager.ACTION_OK){
                        _this.startScene({
                            "option" : W.SceneManager.OPTION_NEW,
                            "sceneName" : "scene/ProfileAddScene"
                        });
                    }
                }
            },
