 ...
 */
W.defineModule([ "mod/Util", "comp/my/Bookmark", "comp/CouponInfo"],
    function(util, bookmark, couponInfoComp) {
        var STATE_TOP = 0;
        var STATE_BANNER = 1;
        var STATE_MENU = 2;
        ...
        return W.Scene.extend({
            onCreate : function(param) {
                ...
                currComponent = bookmark.getNewComp();
                _currComponent = currComponent.create(null, _this, {mode : 2});
            ...
            onKeyPressed : function(event) {
                ...
                switch (event.keyCode) {
                    ...
                    case W.KEY.BACK:
                        if(state == STATE_COMP){
                            if(state == STATE_COMP){
                                var returnVal = currComponent.operate(event);
                                if(!returnVal) _this.backScene();
                            }else if(state == STATE_TOP){
                            }
                        }else if(state == STATE_TOP){
                        }
